...
async def search_responses(
    ...
    """
    Search agent responses by tags and keywords with formatted markdown content.
    ...
    """
    from .utils.markdown_converter import format_agent_response_for_display

    search_terms = [term.strip() for term in q.split(",") if term.strip()]
    responses = await search_responses_by_tags(
        search_terms, user_id=user_id, limit=limit
    )

    # Format each response with markdown converter
    formatted_responses = []
...
async def submit_feedback(feedback: FeedbackModel) -> FeedbackResponse:
    ...
    try:
        ...
        return FeedbackResponse(
            success=False, message=f"Error storing feedback: {str(e)}"
        )


@app.get("/feedback")
async def get_feedback_list(
    user_id: Optional[str] = None, category: Optional[str] = None, limit: int = 50
):
    """
...
async def upload_document(
    ...
    try:
        ...
        if not document_repository.is_available():
            raise HTTPException(
                status_code=503, detail="Document repository service unavailable"
            )
...
async def list_documents(
    user_id: Optional[str] = Query(None), limit: int = Query(50, le=100)
):
    """
    ...
    try:
        ...
        if not document_repository.is_available():
            return {
                "documents": [],
                "message": "Document repository service unavailable",
            }
...
async def get_feedback_file(file_id: str):
    ...
    try:
        ...
        if not cloud_storage.is_available():
            raise HTTPException(
                status_code=503, detail="Cloud storage service unavailable"
            )
    ...
    try:
        ...
        return {
            ...
            "message": (
                ...
                if result["success"]
                else result.get("error", "Processing failed")
            ),
            "result": result,
        }
...
async def list_processed_feedback_files():
    ...
    try:
        ...
        return {"success": True, "processed_files": files, "count": len(files)}
...
async def get_feedback_pipeline_status():
    ...
    try:
        ...
        processing_rate = (
            (total_processed / total_uploaded * 100) if total_uploaded > 0 else 0
        )
        ...
        status = {
            ...
            "status": (
                ...
                else "warning" if processing_rate > 50 else "unhealthy"
            ),
            ...
            "feedback_statistics": {
                ...
                "categories_distribution": validation.get(
...
Need to emphasize the business value and ROI more clearly. Connect technical features to customer benefits and revenue opportunities.
        ...
        processed_files = await cloud_storage.list_processed_files()
        test_processed = any(
            f.get("original_file_id") == file_id for f in processed_files
        )
        ...
        return {
            ...
            "test_results": {
                ...
                "dominant_category": processing_result.get(
...
async def simple_report_generator_page(request: Request):
    return templates.TemplateResponse(
        "simple_report_generator.html", {"request": request}
    )
...
async def generate_report(request: ReportGenerationRequest) -> ReportResponse:
    ...
    try:
        ...
        from .reports.report_generation_service import bounded_generate_section, generate_section
        import uuid
        from datetime import datetime

        # Simple in-memory cache for section responses
        section_cache = {}
        ...
        if request.use_parallel_processing:
            # Parallel generation with semaphore to limit concurrency (e.g., max 4)
            semaphore = asyncio.Semaphore(4)

            tasks = [
                bounded_generate_section(section, semaphore, orchestrator, request, app.state, section_cache)
                for section in market_research_sections
            ]
            updated_sections = await asyncio.gather(*tasks)

            # Update original sections
            for updated in updated_sections:
                for original in market_research_sections:
                    if original.section_id == updated.section_id:
                        original.content = updated.content
                        original.status = updated.status
                        original.agent_contributors = updated.agent_contributors
                        original.last_updated = updated.last_updated
                        break
        else:
            # Sequential fallback with error handling
            for section in market_research_sections:
                try:
                    await generate_section(section, orchestrator, request, app.state, section_cache)
                except Exception as e:
                    logger.error(f"Error generating section {getattr(section, 'title', 'unknown')}: {e}")
        ...
        report = ReportModel(
            ...
            or "Calnetix Defense & Power Systems Market Intelligence Report",
...
async def list_reports(
    user_id: Optional[str] = None, status: Optional[str] = None, limit: int = 50
):
    """
...
async def revise_section_with_feedback(
    report_id: str, section_id: str, feedback_request: SectionUpdateRequest
):
    """
    ...
    try:
        ...
        # Create a revision prompt using context injector
        revision_prompt = ContextInjector().section_revision_prompt(
        ...
        revised_content = orchestrator_response.get(
            "response", section.get("content", "")
        )

        # Update the section
        success = await update_report_section(
            report_id, section_id, revised_content, "completed"
        )
...
async def generate_market_research(request: Request):
    ...
    try:
        ...
        if not topic or not template:
            raise HTTPException(
                status_code=400, detail="Topic and template are required"
            )
